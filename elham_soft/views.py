...
def create_app(request):
    shutil.copyfile(
        os.path.join(BASE_DIR,"opera_soft/options.py"),
        os.path.join(BASE_DIR,"app/options.py"),
    )

    content_models = """from django.db import models\n\n\n"""
    for model in models.Model.objects.all():
        content_fields = ""
        for field in models.Field.objects.filter(model=model):
            content_attrs = ""
            for attr_name, attr_value in getattr(models.models,field.field_type)().__dict__.items():
                if hasattr(field,attr_name):
                    field_attr_value = getattr(field,attr_name)
                    if not attr_name == "name" and not attr_value and attr_value != field_attr_value:
                        attr_type = type(field_attr_value)
                        if attr_type == str:
                            field_attr_value = f"'{field_attr_value}'"
                        content_attrs += rf"{attr_name}={field_attr_value},"
            content_fields += f"""    {field.name} = models.{field.field_type}({content_attrs})\n"""
        content_models += f"""class {model.name}(models.Model):\n{content_fields}\n\n"""

    with open(os.path.join(os.path.join(BASE_DIR,"app"),"models.py"),"w",encoding="utf-8") as _models_file:
        _models_file.write(content_models)


    content_admin = """from django.contrib import admin\nfrom .options import ModelSetting, ModelView, ModelReport\n"""
    content_admin += """from .options import AdminSite\n"""
    content_admin += """from .import models\n"""

    content_urls = """from django.urls import path,include\n"""
    content_urls += """from . import admin\n\n\n"""

    content_urls += """urlpatterns = [\n"""

    for site in models.Site.objects.all():
        content_admin += f"""class {site.name}Site(AdminSite):\n    pass\n"""
        content_admin += f"""{site.name}_admin = {site.name}Site(name='{site.name}')\n"""

        content_urls += f"""    path('{site.name}/',admin.{site.name}_admin.urls),\n"""

        for setting in models.Setting.objects.filter(site=site):
            content_admin += f"""class {setting.model.name}Setting(ModelSetting):\n    pass\n"""
            content_admin += f"""{site.name}_admin.register(models.{setting.model.name},{setting.model.name}Setting)\n"""
    content_urls += """]"""
    with open(os.path.join(os.path.join(BASE_DIR,"app"),"admin.py"),"w",encoding="utf-8") as file_admin:
        file_admin.write(content_admin)
    with open(os.path.join(os.path.join(BASE_DIR,"app"),"urls.py"),"w",encoding="utf-8") as file_urls:
        file_urls.write(content_urls)
